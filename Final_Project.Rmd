...
Feature engineering was one of the most important parts of this project. When approaching this, we had to consider what type of information was the most important to determine when predicting a big play. Drawing from our background in fantasy football, we decided that the AYP: Average Yards Per Play, Age of the Player, Yards to Goal, and Points difference. 

Average yards per play: This would be the biggest indicator of player performance since it essentially shows how well they do in a rushing play. This required time manipulation. 

Age of the Player: There's a tradeoff between experience and being too old for the position that would offer a lot of information. This also required time manipulation. 

Yards to Goal: Coaches have different strategies and levels of aggression based on exactly how far they are from a touchdown. For example, in the red zone, they might try to take a bigger risk and run around the side. When further away, a coach might want the rusher to go down the middle for a safer, but lower yardage. 

Points difference: This also plays into coach strategy. Depending on how close the game is, a coach might make a safer or riskier play call. 

Additionally, we parsed the offense and defense personnel to find the number for each position. This required some string manipulation. 
...
We had already done a lot of exploratory data analysis in the cleaning section, but now we wanted to visualize the relationship between different predictors and the response. First we just looked at yards and the explanatory variables. The correlation between Yards and Good was very high, but that should be disregarded because Good was created from Yards. Acceleration, YardsToGoal, Speed, Distance, and Yardline look like they have some sort of relationship. Defenders in the box also had a very stong negative correlation with Yards, which makes logical sense. Next, we made a correlation plot which  shows very weak relationships between the majority of variables. Temperature and week seem to be highly correlated, which is due to certain weeks being routinely hotter than others. When building our regression model we should choose to include only one of those two variables since this multicolinearity violates the regression assumption of independence. Other areas we see this is the distance and the down. NFLID rushes and NFL ID would not be included in the regression. Scores are correlated with quarter as well, since as the quarter increases the score increases. 
...
# Determining correlation: correlation only works with quantitative variables, so I remove the NAs and the qualitative variables
...
only_quant = clean %>% drop_na() %>% select_if(sapply(., class) %in% c("numeric", "double", "integer")) %>% select(-wr, -te, -rb, -dl, -lb, -db)
...
sd(test$Yards)
diffcomb
# The standard deviation is 6.01 yards, whereas our predictions were off by an average of less than 1.52 yards, meaning that they deviate by an average of 1/4 of one standard deviation, a mark with which we were very satisfied.
```
Ultimately we spent the bulk of our time understanding the data and engineering features, and chose to go for depth rather than breadth on the models, but it paid off here.
